...
From a connectivity perspective scientists might rely on

\begin{itemize}
  \item cable network connections with high bandwidth and reliability as e.g. provided by the employing research institute
  \item wireless network connections with varying stability and bandwidth as e.g. available at home
  \item low-bandwidth, instable, disconnective (and port-disabled) wireless network connection such as e.g. on mobile phone, train and/or airport WiFi hotspots
\end{itemize}

From a HPC system hardware type perspective scientists might require
...
From a workflow perspective scientists have to

\begin{itemize}
  \item work on several clusters provided by one HPC center if compute task types and compute project memberships require them to
  \item rely only on a single-purpose Jupyter notebook session
  \item be able to do advanced workflow management\footnote{for example with \href{https://www.nextflow.io/}{Nextflow}, \href{https://snakemake.readthedocs.io/}{Snakemake}, ...}
\end{itemize}

\subsection{previous text}
