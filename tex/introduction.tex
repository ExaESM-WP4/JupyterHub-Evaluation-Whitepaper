...
\begin{itemize}
  \item are happy with a ready-to-use and sufficiently recent \href{https://www.anaconda.com/products/individual}{Anaconda} data science package stack.
  \item want to use a similar and/or the exact software package stack that they already use on their desktop computer and/or a different HPC or in-house compute cluster.
  The possibility of bringing already expedient software environments would strongly enhance single user scientific productivity.
  The pecularities of a working environment are then already known to the scientist and they themselves are enabled to decide at what point of the scientific project the risks that come with updating a software stack can be taken.
  It would also enable and increse repeatability and reproducibility of scientific results.
  \item require a version-specific and very unique software package set.
  In the rapidly developing open source software world, but probably only in rare cases, the inevitable introduction of breaking changes might make a perfectly valid piece of scientific code largely unusable.
  Human resources to follow-up with recent developments might not be available especially towards the end of a research project, which makes enabling the usage of version-specific software environments a strong requirement for scientific user productivity on a system.
  \item strongly profit from system-optimized packages.
\end{itemize}

From a general workflow perspective scientists might
